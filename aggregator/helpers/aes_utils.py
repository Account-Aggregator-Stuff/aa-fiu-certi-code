...
def generate_redirect_url(fi, requestor_type, txnid, sessionid, userid, redirect, srcref, aes_token):
    global SECRET_KEY
    SECRET_KEY=aes_token
    now_utc = datetime.now(timezone.utc)
    salt = now_utc.strftime("%d%m%Y%H%M%S") + now_utc.strftime("%f")[:1]  # Use reqdate as salt

    fi = "pyro-bank-fiu"  # Replace with your FI identifier
    requestor_type = "FIU"  # Assuming requestor type is FIU, replace if different
    
    # Encrypt fi and requestorType using XOR and Base64, with reqdate as the key
    ...
    base_url = "https://reactjssdk.finvu.in/" # Replace with actual AA domain
